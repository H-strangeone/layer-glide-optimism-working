...
export default function Index() {
  ...
  useEffect(() => {
    ...
    const ctx = gsap.context(() => {
      ...
      // Network & Actions reveal
      gsap.fromTo(networkRef.current, { opacity: 0, y: 30 }, { 
        opacity: 1, y: 0, duration: 0.7, 
        scrollTrigger: { trigger: networkRef.current, start: 'top 85%' } 
      });
      gsap.fromTo(actionsRef.current, { opacity: 0, y: 30 }, { 
        opacity: 1, y: 0, duration: 0.7, 
        scrollTrigger: { trigger: actionsRef.current, start: 'top 85%' } 
      });
    }, containerRef);
    return () => ctx.revert();
  }, [isConnected]);

  // Smoother transition effect for actions
  useEffect(() => {
  ...
  return (
    <div ref={containerRef} className="space-y-32 pb-32">
      ...
      {/* ── ACTIONS TABBED (Natural feel, no pink, smooth transitions) ── */}
           ...
           <div className="flex bg-white/5 p-1 rounded-sm border border-white/5">
              {[
                ...
                <button
                  ...
                  className={`flex items-center gap-2 px-6 py-2 rounded-sm text-xs font-black uppercase tracking-[0.1em] transition-all duration-400 ${
                    activeAction === tab.id 
      ...
      {/* ── TRACKER ── */}
      {address && (
          <section className="space-y-8 tracker-block">
      ...
      <section className="grid grid-cols-2 lg:grid-cols-3 gap-6">
          {FEATURES.map((f, i) => (
             <div key={i} className="p-8 bg-white/5 border border-white/5 rounded-sm group hover:border-orange/20 transition-all duration-300">
